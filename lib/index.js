...
Cacher.prototype.key = function key() {
  ...
  if (this.q) {
    ...
    .update(this.q)
    .digest('hex');
    return [this.cachePrefix, this.options.cacheKey, '__raw__', 'query', hash].join(':');
  }
  hash = crypto.createHash('sha1')
  .update(CircularJSON.stringify(this.options, jsonReplacer))
  .digest('hex');
  return [this.cachePrefix, this.options.cacheKey, this.modelName, this.method, hash].join(':');
};
